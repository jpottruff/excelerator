...
import * as mockParsingData from '../../shared/mock-data/parsing-variables';
...
export class ParsingService {
  ...
  getRelevantDataFromJSON(JSONfile) {
    return new Promise ( resolve => {
      ...
      incomingData.forEach( (dataRow, i) => {
        if (mockParsingData.dataToFilterByList.includes((dataRow[mockParsingData.columnToFilter]))) {
          const rowNum = i + 2;
  ...
  getRelevantDataFromRow(row) {
    ...
    row.forEach((dataItem, i) => {
      if (mockParsingData.columnsToInclude.includes(i)) {
